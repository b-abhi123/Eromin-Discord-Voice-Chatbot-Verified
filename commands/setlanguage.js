...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('setlanguage')
		.setDescription('Set the language of Eromin.')
	...
	async execute(interaction) {
		...
		server.findOne({ Server: interaction.guildId }, async(err, data) => {
			if (data){
				...
				interaction.reply(`Your server language is now: **${language}**, i.e. ${ISO6391.getName(language)}\nPlease set the language to **en** in order to return to default.`)
			} else {
				new server({
